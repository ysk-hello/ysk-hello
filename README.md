# 👨‍💻 About Me

エンジニアとしてソフトウェアを開発するかたわら、技術書の執筆活動を行っています。

- 💻 **C# / WinUI 3** を用いたモダンなWindowsアプリ開発がメイン領域です。
- 📖 **Kindle** でプログラミングに関する技術解説書をセルフパブリッシングしています。

### 📚 My Books (Kindle)
- [📗 C# WinUI 3 デスクトップアプリ開発 完全ガイド](https://www.amazon.co.jp/dp/B0H6WFV4KV)
- [📗 動かして学ぶ Entity Framework Core（EF Core）実践入門: C#／.NET のデータアクセスを基礎からパフォーマンス改善まで](https://www.amazon.co.jp/dp/B0H8PLJD3D)
- [📗 Supabase 実践入門 ― Vue.js で作る TODO アプリ](https://www.amazon.co.jp/dp/B0H7MJ9XSR)

👉 **[Amazon著者セントラルはこちら](https://www.amazon.co.jp/stores/ysk-hello/author/B0H8SHL78T)**

### 🚀 Technologies
`C#` `WinUI 3` `.NET` `SQLite` `Entity Framework Core`
